...
if ($_SERVER['REQUEST_METHOD'] === 'POST') 
{
    ...
    if (empty($donation_type)) {
        $errors[] = "Please select a donation type.";
    }
    
    if (empty($allocation)) {
        $errors[] = "Please select an allocation.";
    }
    ...
    if (empty($errors)) 
    {
        try 
        {
            $stmt = $pdo->prepare("INSERT INTO donations (name, email, donation_type, allocation, amount, message, payment_method, donation_date) 
                                  VALUES (?, ?, ?, ?, ?, ?, ?, NOW())");
            $stmt->execute([$name, $email, $donation_type, $allocation, $amount, $message, $payment_method]);

            $_SESSION['success'] = "Thank you for your donation of Ksh" . number_format($amount, 2) . "!";
